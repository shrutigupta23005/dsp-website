...
export default function ShareButtons({ productUrl, productName }: ShareButtonsProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {/* WhatsApp */}
      <a
        href={buildShareUrl("whatsapp", productUrl, productName)}
        target="_blank"
      ...
      {/* Facebook */}
      <a
        href={buildShareUrl("facebook", productUrl, productName)}
        target="_blank"
        rel="noopener noreferrer"
        className="flex h-7 w-7 items-center justify-center rounded-full transition-transform hover:scale-110"
      ...
      >
        {copied ? (
          <Check className="h-3.5 w-3.5" style={{ color: "var(--success-color)" }} />
        ) : (
